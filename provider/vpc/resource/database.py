        ...
        if len(response["DBInstances"]) > 0:
            ...
            for data in response["DBInstances"]:
                if data['DBSubnetGroup']['VpcId'] == self.vpc_options.vpc_id:
                    rds_digest = ResourceDigest(id=data['DBInstanceArn'], type='aws_db_instance')
                    subnet_ids = []
                    for subnet in data['DBSubnetGroup']['Subnets']:
                        subnet_ids.append(subnet['SubnetIdentifier'])
                        self.relations_found.append(ResourceEdge(from_node=rds_digest,
                                                                 to_node=ResourceDigest(id=subnet['SubnetIdentifier'],
                                                                                        type='aws_subnet')))

                    resources_found.append(Resource(digest=rds_digest,
        ...
        if len(response['CacheClusters']) > 0:
            ...
            for data in response['CacheClusters']:
                ...
                if cachesubnet['CacheSubnetGroups'][0]['VpcId'] == self.vpc_options.vpc_id:
                    ec_digest = ResourceDigest(id=data['CacheClusterId'], type='aws_elasticache_cluster')
                    subnet_ids = []
                    for subnet in cachesubnet['CacheSubnetGroups'][0]['Subnets']:
                        subnet_ids.append(subnet['SubnetIdentifier'])
                        self.relations_found.append(ResourceEdge(from_node=ec_digest,
                                                                 to_node=ResourceDigest(id=subnet['SubnetIdentifier'],
                                                                                        type='aws_subnet')))

                    resources_found.append(Resource(digest=ec_digest,
    ...
    def get_resources(self) -> List[Resource]:
        ...
        if len(response['DBInstances']) > 0:
            ...
            for data in response['DBInstances']:
                ...
                if data['DBSubnetGroup']['VpcId'] == self.vpc_options.vpc_id:
                    docdb_digest = ResourceDigest(id=data['DBInstanceArn'], type='aws_docdb_cluster')
                    subnet_ids = []
                    for subnet in data['DBSubnetGroup']['Subnets']:
                        subnet_ids.append(subnet['SubnetIdentifier'])
                        self.relations_found.append(ResourceEdge(from_node=docdb_digest,
                                                                 to_node=ResourceDigest(id=subnet['SubnetIdentifier'],
                                                                                        type='aws_subnet')))
                    resources_found.append(
                        Resource(digest=docdb_digest,
                                 name=data["DBInstanceIdentifier"],
